%------------------------------------------------------------------------------
% File     : LB347+1
% Domain   : Inference Rules (LogicBench)
% Problem  : MP chain depth 11 var 2
% Status   : Theorem
% Rating   : 0.44 v1.0.0
% Source   : [LogicBench]
% SPC      : FOF_THM
%------------------------------------------------------------------------------
fof(s1, axiom, ( ! [X] : ((omega(X) => sigma(X))) )).
fof(s2, axiom, ( ! [X] : ((sigma(X) => rho(X))) )).
fof(s3, axiom, ( ! [X] : ((rho(X) => tau(X))) )).
fof(s4, axiom, ( ! [X] : ((tau(X) => nu(X))) )).
fof(s5, axiom, ( ! [X] : ((nu(X) => mu(X))) )).
fof(s6, axiom, ( ! [X] : ((mu(X) => xi(X))) )).
fof(s7, axiom, ( ! [X] : ((xi(X) => alpha(X))) )).
fof(s8, axiom, ( ! [X] : ((alpha(X) => beta(X))) )).
fof(s9, axiom, ( ! [X] : ((beta(X) => gamma(X))) )).
fof(s10, axiom, ( ! [X] : ((gamma(X) => delta(X))) )).
fof(s11, axiom, ( ! [X] : ((delta(X) => epsilon(X))) )).
fof(base, axiom, ( omega(obj1) )).
fof(goal, conjecture, ( epsilon(obj1) )).
%------------------------------------------------------------------------------
